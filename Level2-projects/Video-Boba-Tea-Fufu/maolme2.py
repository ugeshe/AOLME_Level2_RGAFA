...
class TSP:   
    ...
    def plot_tour(self, tour, num_of_rows, num_of_cols):
        ...
        # TODO : ????? TEST TOUR for validity.
        
        # Create a white image:
        # ??? FIX dimensions later        
        img = make_img(num_of_rows, num_of_cols,"FFFFFF")
        plt.figure()
        plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    
        plt.figure()
        plt.ylim(float(num_of_rows), 0.0)
        plt.axis('image')
        for i in range(0, len(tour)-1, 1):            
            j1 = tour[i]-1
            j2 = tour[i+1]-1
            ...
            c1 = (c1[1], c1[0])
            c2 = (c2[1], c2[0])
            img = cv2.circle(img, c1, 1, (0,0,255))
            img = cv2.circle(img, c2, 1, (0,0,255))
            img = cv2.line(img ,c1, c2,(255,0,0), 1)            
            plt.plot((c1[0], c2[0]), (c1[1], c2[1]), 'b-')
            
        plt.ylim(float(num_of_rows), 0.0)
        plt.xlim(0.0, float(num_of_cols))
        plt.show()
        plot_img(img, "Tour")
...
class spr:
    ...
    def set_sprite(self, sprite_name):
        ...
        # Size also:
        read_num_of_rows, read_num_of_cols, col = sprite_img.shape 
        
        # ????
        # Determine start_row, end_row, start_col, end_col
        # num_of_rows, num_of_cols.
        r1 = range(start_row, end_row+1)
        r2 = range(start_col, end_col+1)
        self.sprite_img = read_sprite_img[r1,r2]
        self.sprite_roi = read_sprite_roi[r1,r2]
        self.sprite_rec = [0, 0, num_of_rows, num_cols] 
...
class vid:
    ...
    def rotate_sprite(self, sprite_name, row_center, col_center, rot_angle):
        ...
        newColCenter=int(round(M[0, 0]*colRECcenter + M[0, 1]*rowRECcenter + M[0, 2]))
        newRowCenter=int(round(M[1, 0]*colRECcenter + M[1, 1]*rowRECcenter + M[1, 2]))
        
        
        # 8. Calculate new boundaries of REC "roiaxis"
        RECaxisRotated=[int(newRowCenter-round(nHREC/2)),int(newColCenter-round(nWREC/2)),int(newRowCenter-round(nHREC/2)+nHREC),int(newColCenter-round(nWREC/2)+nWREC)]
