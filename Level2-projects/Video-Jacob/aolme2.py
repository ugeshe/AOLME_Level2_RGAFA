...
class vid:
    ...
    def place_obj(self, sprite_name,objfull, objroi,objaxis, place_row, place_col):
    ...
    def place_sprite(self, sprite_name, start_row, start_col):
        ...
        newCoord = self.place_obj(sprite_name,objfull, objroi, roiaxis, place_row, place_col)
...
#        newRowCenter=int(round(round(M[1, 0]*colRECcenter,3) + round(M[1, 1]*rowRECcenter,3) + M[1, 2]))
        ...
        newCoord = self.place_obj(sprite_name,objFullRotated, objRoiRotated, RECaxisRotated, row_center, col_center)
    ...
    def scale_sprite(self, sprite_name, row_center, col_center, scale_factor):
        ...
        newCoord = self.place_obj(sprite_name,objResized, maskResized, r, row_center, col_center)
    ...
    def flip_hor(self, sprite_name, place_row, place_col):
        ...
        # 2. Call place object function (.place_obj())
        newCoord = self.place_obj(sprite_name,objfullFlipHor, objroiFlipHor, objaxisFlipHor, place_row, place_col)
    ...
    def flip_ver(self, sprite_name, place_row, place_col):
        ...
        # 2. Call place object function (.place_obj())
        newCoord = self.place_obj(sprite_name,objfullFlipVer, objroiFlipVer, objaxisFlipVer, place_row, place_col)
    ...
    def draw_path(self, sprite_name):
        ...
        try: 
            ...
            if  len(path_coordinates) == 1:
                print('Sprite has not moved, only 1 position!')
                print("\n")
                sys.exit(0)
            
            for i in range(len(path_coordinates)-1):
               imdp=self.video[-1]
               imdp = cv2.line(imdp,path_coordinates[i],path_coordinates[i+1],(255,0,0),5)
            plot_img(imdp,"Path of {}".format(sprite_name))
        except:
