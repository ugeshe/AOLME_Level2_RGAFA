...
img = load_img("neighborhood.png") # Background image file
...
my_1st_sprite.set_sprite("fix it felix") #it from the file B1.pickle
my_1st_sprite.show("fix it felix")       # Show it


# Add a SECOND SPRITE
my_2nd_sprite = spr()             # Construct a sprite
my_2nd_sprite.set_sprite("sb burger") # Load it from the file B1.pickle
my_2nd_sprite.show("sb burger")       # Show it
...
row_bowser, col_bowser = my_video.get_sprite_loc("fix it felix")
my_video.place_sprite("sb burger", row_bowser, col_bowser)
...
# Rotate Mario
row, col = my_video.get_sprite_loc("sb burger")
for ang in range(0, 360, 10):
    my_video.set_new_frame('back2', top_offset, left_offset) 
    my_video.rotate_sprite("sb burger", row, col, ang)

# Grow Mario
sc = 1.0
for scalings in range(1,4,1):
    my_video.set_new_frame('back2', top_offset, left_offset) 
    my_video.scale_sprite("sb burger", row, col, sc)
    sc += 0.125

# Move Mario
for moves in range(1,4,1):
    my_video.set_new_frame('back2', top_offset, left_offset) 
    my_video.move_sprite("sb burger", 10, 0)

################################################
# Draw the video path:
my_video.draw_path("sb burger")

################################################
# TSP material
the_path = my_video.get_path("sb burger")
my_TSP = TSP(the_path)
my_TSP.best_tour()
